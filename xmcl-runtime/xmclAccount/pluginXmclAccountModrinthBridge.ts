...
export const pluginXmclAccountModrinthBridge: LauncherAppPlugin = (app) => {
  ...
  const bridge = () => {
    ...
    pendingAttempt = app.registry
      ...
      .catch((error) => {
        logger.warn(
          'Failed to bootstrap Nebula account from Modrinth authentication.',
  ...
  void app.registry
    ...
    .catch(() => {
      logger.warn('Unable to initialize Modrinth credentials for Nebula Nebula account bootstrap.')
    })
}
