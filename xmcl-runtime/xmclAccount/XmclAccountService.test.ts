...
describe('XmclAccountService Microsoft bootstrap', () => {
  ...
  it('authorizes the Nebula Microsoft identity interactively without a game account login', async () => {
    Object.defineProperty(process, 'platform', { value: 'win32', configurable: true })
  ...
  it('persists the rotated credential when the subsequent account snapshot fails', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      'Nebula session refreshed; account snapshot refresh will retry later.',
    )
  })
})
...
describe('XmclAccountService automatic session refresh', () => {
  ...
  it('clears terminal session state even when secure storage cleanup fails', async () => {
    ...
    expect((service as any).state.account).toBeUndefined()
    expect(logger.warn).toHaveBeenCalledWith(
      'Nebula session cleared in memory, but secure storage cleanup failed.',
    )
  })
})
...
describe('XmclAccountService provider bootstrap queue', () => {
  ...
  it('keeps a successful session when the post-auth identity refresh fails', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      'Nebula authentication succeeded; account identities will refresh later.',
    )
  })
  ...
  it('does not send an expired Nebula credential during provider re-bootstrap', async () => {
    const { service } = createXmclService(false)
    const expired = {
