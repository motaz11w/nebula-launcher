  ...
  implements IXmclAccountService
{
    ...
    this.api = new XmclAccountApi(
      ...
      async () => {
        ...
        return resolveXmclApiEndpoints(
          ...
          () => app.getLogger('ApiBaseUrl').warn('Ignoring invalid xmclApiUrl flight; using default Nebula API origins.'),
        ).common
      },
      () => this.getDpopKey(),
      async () => {
        ...
        return resolveXmclApiEndpoints(
          ...
          () => app.getLogger('BillingApiBaseUrl').warn('Ignoring invalid xmclBillingApiBaseUrl flight; using the default Nebula API origin.'),
        ).common
      },
    )
  ...
  async bootstrapMicrosoft(userId: string): Promise<XmclMicrosoftBootstrapResult> {
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'MicrosoftOAuthSlientFailed') {
        this.warn('Nebula account bridge is waiting for Microsoft Graph consent.')
  ...
  async refreshSession(): Promise<void> {
    ...
    try {
      ...
      this.warn('Nebula session refreshed; account snapshot refresh will retry later.')
    }
  }
  ...
  private async applyAuthResult(result: XmclAuthResult, expectedGeneration = this.sessionGeneration) {
    ...
    if (result.identities === undefined) {
      ...
      try {
        ...
        this.warn('Nebula authentication succeeded; account identities will refresh later.')
      }
    }
  }
  ...
  private async getValidCredential(): Promise<XmclSessionCredential | undefined> {
    ...
    try {
      ...
      if (fallback && Number.isFinite(fallbackExpiresAt) && fallbackExpiresAt > Date.now()) {
        this.warn('Nebula session refresh failed; the current access token will be used until expiry.')
        return fallback
      }
      this.warn('Nebula session refresh failed and the current access token is expired.')
      return undefined
    }
  }
  ...
  private async loadDpopKey(stored: StoredXmclSession): Promise<XmclDpopKey | undefined> {
    ...
    if (embeddedKey) {
      try {
        ...
        this.warn('Nebula DPoP key migration could not create the dedicated copy.')
      }
    }
    return embeddedKey
  }
  ...
  private removeEmbeddedDpopKey(stored: StoredXmclSession) {
    ...
    return this.app.secretStorage
      ...
      .catch(() => {
        this.warn('Nebula DPoP key migration retained the legacy embedded copy.')
      })
  }
  ...
  private clearSession(expectedSessionId?: string) {
    return this.enqueueSessionMutation(async () => {
      ...
      try {
        ...
        this.warn('Nebula session cleared in memory, but secure storage cleanup failed.')
      }
      return true
    })
  }
  ...
  private recordError(error: unknown) {
    ...
    this.state.operationError({
      code,
      message: 'Nebula account request failed',
    })
  }
}
