...
export const pluginXmclAccountMicrosoftBridge: LauncherAppPlugin = (app) => {
    ...
    const attempt = app.registry
      ...
      .then((result) => {
        if (result === 'pending-consent') {
          logger.log('Nebula nebula bridge is waiting for Microsoft Graph consent.')
        }
      })
      .catch((e) => {
        logger.warn('Failed to bootstrap Nebula account from Microsoft authentication.', e)
      })
      .finally(() => {
  ...
  void app.registry
    ...
    .catch(() => {
      logger.warn('Unable to load Microsoft users for Nebula Nebula account bootstrap.')
    })
}
